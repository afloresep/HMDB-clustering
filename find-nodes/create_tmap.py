...
def hierarchy(pkl_file_name):
    ...
    print('neighbors per node df saved as  ', flush=True)
    ...
    index_to_idnumber = {int(row[0]): row[1] for row in original_csv[['idnumber']].itertuples()}
    ...
    first.to_csv('hierarchy-with-names.csv')
...
def move(output_folder):
    ...
    shutil.move('hierarchy.csv', os.path.join(new_directory, 'hierarchy-with-names  .csv'))
...
def main():
    ...
    parser.add_argument("--cfg_node", type=float, default=1/30, help="node_size for layout configuration (default: 1/30)")
